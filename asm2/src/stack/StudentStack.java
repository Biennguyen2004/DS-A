...
public class StudentStack {
   ...
   public void updateStudent(int id, String newName, double newMarks) {
    ...
    public void binarySearchById(int id) {
        // Chuyển stack sang ArrayList và sắp xếp tăng dần trước khi tìm kiếm
        ArrayList<Student> list = new ArrayList<>(stack);
        mergeSort(list, 0, list.size() - 1, true); // Sắp xếp tăng dần bằng Merge Sort
        ...
        while (left <= right) {
            ...
            if (list.get(mid).getStudentId() < id) {
                left = mid + 1; // ID cần tìm lớn hơn, tìm phía phải
            } else {
                right = mid - 1; // ID cần tìm nhỏ hơn, tìm phía trái
            }
        }
